...
double cosine_distance(Mat feat1, Mat feat2) {
  CHECK(feat1.cols == feat2.cols) << "error happen in cosine_distance"
                                  << feat1.cols << " vs. " << feat2.cols;
...
static uint64_t g_track_id = 0;
FTD_Structure::FTD_Structure(const Mat& transform, int in_cam) {
...
void FindRemain(std::vector<int>& input, std::vector<int>& output, int len) {
  ...
  CHECK((int)(input.size() + output.size()) == len)
      << "error happen in function FindRemain, input size: " << input.size()
      << " out size: " << output.size() << " len: " << len;
  ;
}
  ...
  for (auto ti = tracks.end() - 1; ti >= tracks.begin();) {
    ...
    if ((*ti)->time_since_update > max_age) {
      LOG_IF(INFO, ENV_PARAM(DEBUG_REID_TRACKER))
          << "erase: " << (*ti)->time_since_update << " " << max_age;
      ti = tracks.erase(ti);
    }
    ti--;
  }
...
std::vector<OutputCharact> FTD_Structure::Update(
    uint64_t frame_id, std::vector<InputCharact>& input_characts) {
  __TIC__(update);
  remove_id_this_frame.clear();
  frame_count += 1;
  LOG_IF(INFO, ENV_PARAM(DEBUG_REID_TRACKER)) << "frame " << frame_id;
  //<< plan_pts[0];
  ...
  for (auto ti = tracks.begin(); ti != tracks.end();) {
    (*ti)->Predict();
    auto track_rect = std::get<2>((*ti)->GetCharact());
  ...
  for (auto ici = input_characts.begin(); ici != input_characts.end();) {
    auto rect = std::get<2>(*ici);
    auto ici_score = std::get<3>(*ici);
    // rect = rect & roi_range;
    if (rect.width <= 0.f || rect.height <= 0.f ||
      ...
      std::get<2>(*ici) = rect;
      LOG_IF(INFO, ENV_PARAM(DEBUG_REID_TRACKER))
          << rect << " score: " << ici_score;
      ici++;
    }
  }
  vector<Mat> feats;
  vector<int> feat_idxs;
  for (auto& ic : input_characts) {
    feats.emplace_back(get<0>(ic));
    feat_idxs.emplace_back(get<1>(ic));
  }
  __TIC__(get_dis);
  ...
  for (size_t i = 0; i < tracks.size(); ++i) {
    for (size_t j = 0; j < feats.size(); ++j) {
      ...
      for (size_t h = 0; h < 1u; ++h) {
        auto avr_feats = tracks[i]->GetFeatures();
        // auto ori_feats = tracks[i]->GetFeatures(feat_idxs[j]);
        double cdis = get_euro_dis(avr_feats[h], feats[j]);
        // double cdis =min(get_euro_dis(avr_feats[h], feats[j]),
        // get_euro_dis(ori_feats[h], feats[j]));
  ...
  for (auto& t : tracks) {
    ...
    for (auto& ic : input_characts) {
      auto rect_t = std::get<2>(t->GetCharact());
      auto label_t = std::get<4>(t->GetCharact());
      auto rect_i = std::get<2>(ic);
      auto label_i = std::get<4>(ic);
      neg_iou_score.push_back(
          label_t == label_i ? (1.0f - GetIou(rect_t, rect_i)) : 1.0f);
      // cout << neg_iou_score[neg_iou_score.size() - 1] << " ";
  ...
  if (!unmatch_track.empty() && !unmatch_detect.empty()) {
    for (size_t i = 0; i < feat_dists.size(); ++i) {
      for (size_t j = 0; j < feat_dists[i].size(); ++j) {
        // if (!center_dists[i][j]) feat_dists[i][j] = feat_distance_high
        // + 1.0f;
      }
    }
    feats_assign.clear();
    ...
    for (size_t iii = 0; iii < feats_assign.size(); iii++) {
      if (feats_assign[iii] == -1) continue;
      LOG_IF(INFO, ENV_PARAM(DEBUG_REID_TRACKER))
          << feat_dists[iii][feats_assign[iii]] << " feat dis vs iou:  "
          << (1.0f - neg_iou_scores[iii][feats_assign[iii]]);
      if (feat_dists[iii][feats_assign[iii]] < feat_distance_high ||
          (feat_dists[iii][feats_assign[iii]] < feat_distance_high + 0.1 &&
           (1.0f - neg_iou_scores[iii][feats_assign[iii]]) >= iou_threshold)) {
        match_track.push_back(iii);
        match_detect.push_back(feats_assign[iii]);
      }
    }
  ...
  for (unsigned int i = 0; i < unmatch_detect.size(); i++) {
    ...
    tracks.push_back(std::make_shared<FTD_Trajectory>());
    tracks.back()->Init(transform_, input_characts[unmatch_detect[i]], frame_id,
                        cam);
    tracks.back()->UpdateFeature(feats[unmatch_detect[i]],
                                 feat_idxs[unmatch_detect[i]]);
  }
