...
{
    KernelPriv *kernel_priv = (KernelPriv *)handle->kernel_priv;
    ...
    {
        {
            roi_data.roi[ind].prediction->sub_buffer = newBuf;
            {
                ...
                LOG_MESSAGE(LOG_LEVEL_DEBUG, "ROI: %d, %d, %d, %d\n", ROI.x, ROI.y, ROI.width, ROI.height);
...
)
{
    ...
    LOG_MESSAGE(LOG_LEVEL_DEBUG, "ROI end\n");
    return 0;
}
...
int32_t xlnx_kernel_init (VVASKernel *handle)
{
  ...
  KernelPriv *kernel_priv = new KernelPriv;
  if (!kernel_priv) {
...
{
    if (handle->kernel_priv) {
      delete (KernelPriv*)(handle->kernel_priv);
    }
    return 0;
}
