      ...
      VVASFrame * input[MAX_NUM_OBJECT], VVASFrame * output[MAX_NUM_OBJECT])
  {
    ...
    if (inframe->props.fmt == VVAS_VFMT_Y_UV8_420)
    {
...

      {
        ...
        int64_t ind = (int64_t)infer_meta->prediction->reserved_2;
        vector<TrackerResult> *ids = (vector<TrackerResult>*)(infer_meta->prediction->reserved_5);
        ...
        if (data != NULL) {
          ...
          for (int i = 0; i < data->person_infos.size(); i++) {
